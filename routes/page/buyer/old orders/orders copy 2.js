  ...
  try {
    ...
    if (orders) {
      if (JSON.parse(orders)) {
        ...
        for (let i = 0; i < o.length; i++) {
          ...
          if (!product) {
            return res.json({ error: { msg: `Product with id ${o[i].id} Not Found` } });
          }
    ...
    await OrderModel.insertMany(allOrders, async (err, data) => {
      if (err) {
        ...
        console.log(data, "the fxxxk data ")
        for (let i = 0; i < data.length; i++) {
          ...
          let sender = {
            userType: "buyer",
            id: u.buyer_id,
            message: `You have a new order with id ${u._id}, on your post '${u.product_id}'`,
          };
          let receiver = {
            userType: "seller",
            id: u.seller_id,

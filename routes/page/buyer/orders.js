...
router.post("/", [...checker], async (req, res) => {
  ...
  try {
    ...
    let paid =  await paystack.transaction
    .initialize({
      name: `${bfn} ${bln}`,
      email: bemail,
      amount: ototal,
    ...
    if (paid) {
      if (paid.data) {
        payment_reference = paid.data.reference;
   ...
   let insertedOrder  = await OrderModel.insertMany(insertingOrders, async (err, data) => {
      if (err) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          let ref = new ReferenceModel({
            reference:paid.data.reference,
            data_model:"order"
          })
          ref.save()
        ...
        return res.status(200).json({ success: {
          paid,
          authorization_url:paid.data.authorization_url,
          msg:"Please complete your payment, so your order can be placed "
        } });
      }
    });
  } catch (error) {
